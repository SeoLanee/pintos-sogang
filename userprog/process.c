...
static struct thread *get_child(struct thread *t, tid_t tid);
...
{
  int argc = 0;
  char *argv[CMDMAX] = {NULL,};
  char *file_name = file_name_, *token, *save;
  struct intr_frame if_;
  struct thread *t;
  bool success;

  for(token = strtok_r(file_name, " ", &save); argc < CMDMAX && token != NULL; token = strtok_r(NULL, " ", &save)){
    argv[argc++] = token;
  }
...
get_child(struct thread *t, tid_t tid)
{
  ...
  for(e = list_begin(&t->child_list); e != list_end(&t->child_list); e = list_next(e)){
    ret = list_entry(e, struct thread, c_elem);
    if(ret->tid == tid)
      return ret;
  }
...
setup_stack (void **esp) 
{
  ...
  vme = vm_create_vme();
  frame = frame_alloc (PAL_ZERO, vme); //mark
  if (frame != NULL) 
    {
...
bool process_expand_stack(void *fault_addr)
{
  struct vm_entry *vme;
  struct frame *frame;
  bool success = false;
...
bool handle_mm_fault (struct vm_entry *vme)
{
  ...
  frame = frame_alloc(0, vme);//mark
  if (frame == NULL)
    return false;
      
  switch(vme->type){
    case PAGE_BIN:
      if (!load_file(frame->kaddr, vme)) {
        frame_free_with_page(frame);
        return false;
      }
      break;
    case PAGE_SWAP:
      ...
      swap_in(frame->kaddr, vme->swap_idx);
  ...
  if (!install_page (uaddr, frame->kaddr, vme->writable)) {
    frame_free_with_page(frame);
    return false;
  }
